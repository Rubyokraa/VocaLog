...
function setActiveTab(tabName) {
  ...
  buttons.forEach((b) => {
    ...
    if (isActive) {
      ...
      b.classList.remove("bg-neutral-900/20");
    } else {
      b.classList.remove("bg-neutral-900/60");
      b.classList.add("bg-neutral-900/20");
    }
  });
}
...
function renderCollectionsSelect() {
  ...
  importSelect.value = SYSTEM_COLLECTION_IDS.unsorted;
}
...
function renderLibraryItems() {
  ...
  list.innerHTML = items
    .map((item) => {
      ...
      return `
        <button type="button"
          class="w-full text-left rounded-2xl border border-neutral-900 bg-neutral-900/10 p-4 hover:bg-neutral-900/20 transition ${isSelected ? "ring-1 ring-neutral-600" : ""}">
...
function startListening() {
  ...
  try {
    ...
    renderListenCard();
    startTimer();
    // Best-effort: try to play if we have an audio URL.
    const audioEl = el("audioEl");
    if (audioEl && state.activeSession) {
      const currentItem = getActiveItem();
      if (currentItem?.audioUrl) {
        audioEl
          .play()
          .catch(() => {
            // Ignore autoplay/CORS issues; timer still counts.
          });
      }
    }
  } catch (e) {
      ...
      const ratingBlock = log.rating
        ...
        : log.ratingDeferred
          ? `<div class="mt-1 text-xs text-neutral-500">Saved for later</div>
             <button class="mt-2 text-xs rounded-xl border border-neutral-800 bg-neutral-900/20 text-neutral-100/90 px-3 py-2 hover:bg-neutral-900/35 transition" data-rate="${log.id}">Rate now</button>`
          : `<button class="mt-2 text-xs rounded-xl border border-neutral-800 bg-neutral-900/20 text-neutral-100/90 px-3 py-2 hover:bg-neutral-900/35 transition" data-rate="${log.id}">Rate</button>`;
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="mt-4 flex flex-col sm:flex-row sm:items-center gap-3 sm:justify-between">
                ...
                <div class="flex gap-3">
                  <button class="rounded-xl border border-neutral-800 bg-neutral-900/20 text-neutral-100/90 px-3 py-2 text-xs hover:bg-neutral-900/35 transition" data-edit="${log.id}">
                    Edit duration
                  </button>
                </div>
              </div>
        ...
        <div class="rounded-2xl border border-neutral-900 bg-neutral-900/10 p-4">
          <div class="flex items-start justify-between gap-3">
            ...
            <div class="text-right">
              <button class="rounded-xl border border-neutral-800 bg-neutral-900/20 text-neutral-100/90 px-3 py-2 text-xs hover:bg-neutral-900/35 transition" data-open="${c.id}">Open</button>
              ${isSystem ? "" : `<button class="rounded-xl border border-neutral-800 bg-neutral-900/20 text-neutral-100/90 px-3 py-2 text-xs hover:bg-neutral-900/35 transition" data-del="${c.id}">Delete</button>`}
            </div>
          </div>
        </div>
      `;
        ...
        (item) => `
          <div class="rounded-xl border border-neutral-900 bg-neutral-900/10 p-3">
            <div class="flex items-center justify-between gap-3">
              ...
              <button class="rounded-xl border border-neutral-800 bg-neutral-900/20 text-neutral-100/90 px-3 py-2 text-xs hover:bg-neutral-900/35 transition" data-open-listen="${escapeAttr(
                item.id,
              )}">
                Open in Listen
              </button>
...
function bindUI() {
  ...
  document.querySelectorAll(".tabBtn").forEach((b) => {
    b.addEventListener("click", () => {
      ...
      if (b.dataset.tab === "collections") renderCollectionsList();
    });
  });
